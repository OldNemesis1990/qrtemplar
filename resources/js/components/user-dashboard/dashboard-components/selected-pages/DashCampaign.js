...
class DashCampaign extends Component {
    ...
    getUserQRCampaigns() {
        ...
        .then( (response) => {
            // console.log(response)
            let qr_count = response.data.qr_codes
            this.setState({
                created_campaigns: [...response.data.account_info[0].campaigns],
                qr_codes: [...response.data.qr_codes],
                campaign_limit: response.data.campaign_limit,
    ...
    deleteQr = (e, id, campaign_id, account_id) => {
        console.log(id, campaign_id, account_id)
        if(confirm("Are you sure you would like to delete this QR Code?")) {
            let formData = new FormData()
            formData.append('qr_id', id)
            formData.append('campaign_id', campaign_id)
            formData.append('account_id', account_id)
            
            axios.post(`/delete/qr/${account_id}/${campaign_id}/${id}`)
            .then( (res) => {
                console.log(res)
                this.getUserQRCampaigns()
            } )
        ...
        return(
            <div className="campaign-management">
                ...
                <div className="create-campaign">
                    <div className="row">
                        <div className="col-md-2">
                            {(this.state.campaign_count < this.state.campaign_limit && this.state.qr_count < this.state.qr_limit) && <a href="#" className="create-campaign-item" onClick={(e) => {this.createCampaignTrigger(e)}}>{this.state.form_btn_text} Campaign</a>}
                        </div>
                ...
                <div className="created-campaigns">
                    <h5> {`Used: ${this.state.qr_count} from ${this.state.qr_limit} QR Codes`}</h5>
                                                ...
                                                {this.state.qr_codes.map( (qr) => {
                                                    if(qr.campaign_id == c.id) {
                                                        ...
                                                        const qrHoverEditor = {
                                                            ...
                                                            hover: {overflow: "hidden", opacity: 1, height: "100%"}
                                                        }
                                                        return(
                                                            <motion.div className="qr-slider" intial="initial" whileHover="hover" animate="initial">
                                                                <motion.div className='qrHoverEditor' variants={qrHoverEditor} transition={{type: "spring", bounce: 1, damping: 7, stiffness:150}}>
                                                                    <button className="form-control btn-secondary" onClick={(e) => {this.props.setPage(e,'view-qr-stats', c.id, qr.id), this.setCampaignId(e, c.id)}}>View</button>
                                                                    <button className="form-control btn-primary" onClick={(e) => {this.props.setPage(e,'edit-qr', c.id, qr.id), this.setCampaignId(e, c.id)}}>Edit</button>
                                                                    <button className="form-control btn-danger" onClick={(e) => {this.deleteQr(e, qr.id, qr.campaign_id, this.state.account_id)}}>Delete</button>
                                                                </motion.div>
